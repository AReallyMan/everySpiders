    ...
    def parse_item(self, response):
        ...
        if self.duplicate.redis_db.hexists(self.duplicate.redis_data_dict, response.url):
            ...
            if content:
                ...
                c = response.xpath("//div[@id='con_arc_content']").xpath('string(.)').extract_first()
                item['content'] = ''.join(c)
            publishtime = response.xpath("//div[@class='share yahei cf']/div/div/text()").extract_first()
            publishtime_2 = response.xpath("//div[@class='fs18 r share-date']/text()").extract_first()
            if publishtime:
                item['publishtime'] = publishtime
            elif publishtime_2:
                item['publishtime'] = publishtime_2
            else:
                item['publishtime'] = response.xpath("//div[@class='bigPhoto-source cf ']/div/text()").extract_first()
